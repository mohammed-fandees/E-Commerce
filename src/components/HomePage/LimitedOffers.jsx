            ...
            {timeUnits.map((unit, index) => (
              <div key={unit.label} className="flex items-end gap-1">
                <div className="flex flex-col">
                  <span className="text-[12px] font-medium text-center">{t(`home.offers.countdown.${unit.label}`)}</span>
                ...
                {index < timeUnits.length - 1 && (<span className="text-[#DB4444] text-[32px] font-bold">:</span>)}
              </div>
            ))}
          </div>
        )}
