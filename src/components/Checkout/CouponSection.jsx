...
const CouponSection = ({
  ...
  const handleRemoveCoupon = () => {
    onRemoveCoupon();
    onCouponCodeChange(''); // Clear input after removal
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="flex space-x-2 flex-col sm:flex-row gap-4">
            <input
              ...
              className="flex-1 w-full p-3 bg-[#F5F5F5] rounded-md focus:outline-none border focus:border-[#db4444] transition-colors"
              disabled={loading}
            />
            <Button type="submit">
        ...
        <div className="space-y-3 relative">
          <div className="flex lg:items-center lg:flex-row gap-2 justify-between flex-col items-start p-3 bg-green-50 border border-green-200 rounded-lg">
            ...
            <Button
              ...
              className="absolute !p-1 w-6 h-6 flex justify-center items-center !rounded-full -top-[10px] -start-[10px]"
            >
              x
            </Button>
          </div>
